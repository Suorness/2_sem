...
procedure createarray(enterstring:string; var firstEnter:boolean;
...
{Шифрование}
procedure cypher(CArray:TArD;lengthAr,sim:integer; var status:TStat;
...
begin
  ...
  randomize;
  number:=1;
  writeln('firstEnter status ',status);
...
Begin
  ...
  readln(frontstring);
  firstEnter:=true;
  createarray(frontstring,firstEnter,NArray,lengthAr);
  print(NArray);
  writeln('-------------------------');
  {status:=1;
  direction:=0;
  ...
  createarray(totalstr,firstEnter,totalArray,lengthAr);
  writeln('-------------------------');
