...
Const chr=45;
      null=48;
//   онстанта дл€ проверки на отрицательность
...
{str0-конечный результат
 ...
 а,b,c- промежуточные значени€, которые принимают вид одного символа
 k-переход€щий дес€ток при сложении
 g-разность строк
...
Begin
  ...
  repeat
    ...
    readln(str1);
    if ord(str1[1])=chr then
    writeln('¬ы ввели отрицательное число, введите положительное');
  until ord(str1[1])<>chr;
  repeat
    writeln('¬ведите второе число');
    readln(str2);
    if ord(str2[1])=chr then
    writeln('¬ы ввели отрицательное число, введите положительное');
  until ord(str2[1])<>chr;
  // расчет количества символов
  sim1:=length(str1);
  sim2:=length(str2);
  sim0:=0;
 (*присвоем  максимальное количество символов
   конечной строке и выравниваем количество символов
   во второй строке*)
  if sim1>sim2 then
  begin
    sim0:=sim1;
 ...
 //переворачиваем  первое число
  ...
  begin
    ...
    c:=b+a+k;
    k:=0;
    // проверка на прибавление нового разр€да
    if (i=sim0) and (c>=10) then
    str0:=str0+'1';
    //проверка на переход дес€тка
    if c>=10 then
    begin
      k:=1;
      c:=c-10;
    end;
    //преобразуем число в один символ строки
    str0[i]:=inttostr(c)[1];
  end;
  //переворачиваем число в исходную форму
  ...
  // проверка на нули в начале
  while ord(str0[1])=null do
  begin
    for i:=1 to length(str0) do
    str0[i]:=str0[i+1];
  end;
  writeln('–езультат сложени€');
  writeln(str0);
  readln;
end.
