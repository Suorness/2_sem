...
procedure work (var list:queue;takttime:integer);
begin
...
begin
  ...
  inaction:= takttime-(enterNum mod takttime);               {ТУт баг}
  for i:=1 to 3 do
    add(first,firstlist,enterNum,2,true,3);
  for i:=1 to 2 do
    add(second,secondlist,enterNum,1,true,2);
  repeat
    firstlist:=first^.next;
    Circle_passed:=0;
    NeedaddEl:=true;
    ...
    if NeedaddEl then
      ...
      begin
        ...
        del(first,massfield);
        firstlist:=tempvalue;
      end                                                              {ТУТ МОЖЕМ ТЕРЯТЬ ЭЛЕМЕНТ ПРОТЕСТИРОВАТЬ!!!}
      else
      begin
        tempvalue:=first^.next.next;
