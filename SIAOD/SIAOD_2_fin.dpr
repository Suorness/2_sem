...
Const
  CTERM:array[1..10] of string = {('5','3','2','1','4','6','7','8','9','10');}
  ('Хордовые', 'Позвоночные', 'Млекопитающие', 'Приматы', 'Антропоиды', 'Сфера',
   'Треугольник' , 'Пирамида', 'Эллипс', 'Тор');
  UnTerm:array[1..10] of string =
  ('Ланцетник','Ястреб','Недошар','Человек','Обезьяна','Сложно',
  'Равносторонний Треульгольник','Правильная Пирамида','Полый Эллипс','Тор2');
...
begin
  ...
  begin
    ...
    listTerm^.subtermin.name:='new'+inttostr(i);
    listTerm^.subtermin.base:=listTerm;
    addHashTable(listTerm.subtermin,HashTable);
    new(listTerm^.subtermin.page);
    listTerm^.subtermin.page.nomer:=16+i;
...
begin
  ...
  begin
    ...
    listTerm^.page.nomer:=random(50)+i;
...
procedure sortpage(var firsttermin:termin ;var lost:termin);
var
  i,j:integer;
  count:integer;
  firstEl:termin;
  tempInt:integer;
  tempTermin,tempTerminF,temp,pred:termin;
...
begin
  ...
  listterm:=firstEl;
  setlength(masint,count);
  setlength(mastermin,count);
  new(listterm^.page);
  for i:=0 to count-1 do
  begin
    masInt[i]:=listterm^.page.nomer;
  ...
  for i:=0 to count-2 do
    ...
    begin
      if masInt[j]>masInt[j+1] then
      begin

        tempint:=masint[j+1];
        tempTermin:=masTermin[j+1];
        masint[j+1]:=masint[j];
        masTermin[j+1]:=masTermin[j];
        masint[j]:=tempint;
        masTermin[j]:=tempTermin;
      end;
    end;
    new(tempTerminF);

    new(temp);
    tempTerminF^.next:=temp;

   FOR i:=0 to count-1 do
   begin
    new(temp^.next);
    temp^.next:=masTermin[i];
    temp:=temp^.next;
    temp^.next:=nil;

   end;
   lost:=masTermin[count-1];
   firsttermin:=tempTerminF;
   listterm:=firsttermin;
end;
procedure sortword(var firsttermin:termin ;var lost:termin);
var
  i,j:integer;
  count:integer;
  firstEl:termin;
...
begin
  ...
  for i:=0 to count-2 do
    for j:=0 to count-i-2 do
    begin
      if masWord[j]>masWord[j+1] then
      begin

        tempWord:=masWord[j+1];
        tempTermin:=masTermin[j+1];
        masWord[j+1]:=masWord[j];
        masTermin[j+1]:=masTermin[j];
        masWord[j]:=tempWord;
        masTermin[j]:=tempTermin;
      end;
    ...
    begin
      ...
      begin
        ...
        succses:=true;
          ...
          firsttermin^.next:=firsttermin^.next.next;
...
begin
  ...
  writeln('Команда 3 - Сортировка списка по алфавиту');
          ...
          if strtoint(command) = 5 then
            ...
            if strtoint(command) = 6 then
              ...
              begin
                ...
                begin
                  if strtoint(command) = 3 then
                  begin
                    sortword(firsttermin,lost);
                    ...
                    if strtoint(command) = 0 then
                     ...
                     if strtoint(command) = 8 then
                        sortpage(firsttermin,lost)
                      else
                        main;
                end
